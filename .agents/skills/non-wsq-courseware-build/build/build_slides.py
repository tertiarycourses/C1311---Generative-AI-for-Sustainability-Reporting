...
def test_slide(act_title,text,kicker):
    s=head(slide(),act_title,kicker,TEAL)
    # Verification criteria are deliberately complete and may run to several
    # hundred characters. Give them a full-height card and size by text length
    # so the text never spills below the card or into the footer.
    rect(s,Inches(0.85),Inches(1.9),Inches(11.7),Inches(4.65),DK_PANEL if THEME["dark"] else RGBColor(0xE8,0xF7,0xEE))
    txt(s,Inches(1.2),Inches(2.18),Inches(11),Inches(0.5),[[("✅  Test it",20,DK_GREEN if THEME["dark"] else RGBColor(0x12,0x7A,0x3E),True)]])
    n=len(str(text))
    size=18 if n<=420 else (16 if n<=520 else (14 if n<=620 else 13))
    txt(s,Inches(1.2),Inches(2.9),Inches(11),Inches(3.25),[[(text,size,_ink(),False)]],
...
txt(_dl,Inches(0.85),Inches(6.5),Inches(11.6),Inches(0.4),
    [[(f"Course page:  {getattr(C,'COURSE_URL','https://www.tertiarycourses.com.sg/')}",15,_acc(BLUE),True)]],align=PP_ALIGN.CENTER)
...
for t in C.TOPICS:
    ...
    for a in acts:
        activity_overview(
            f"LAB {a['num']} · {a['duration']} MIN", a["title"], a["desc"], a["build"], a["services"],
        ...
        test_slide(a["title"], a["test"], kicker=f"LAB {a['num']} · {a['duration']} MIN · VERIFY")
    # topic recap
